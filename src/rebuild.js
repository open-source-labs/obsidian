...
export const rebuildFromQuery = async (normalizedQuery) => {
    ...
    for (const primaryField of primaryFieldsArray){
        ...
        const hashName = primaryField.name.value + JSON.stringify(primaryField.arguments)+ JSON.stringify(primaryField.directives);
        const responseKeyName = (primaryField.alias? primaryField.alias.value : primaryField.name.value)
...
for (const cachedObject of cachedArray){
    const returnObject = {};
    console.log("this is cachedObject in rebuildArrays outer loop: ",cachedObject);
    for (const queryField of queryArray){

        const fieldValue = await cacheReadObject(cachedObject, queryField.name.value);
        if (fieldValue===undefined) return undefined;
        console.log (`In ${cachedObject}. ${queryField.name.value} :`,fieldValue)
        if(Array.isArray(fieldValue)){
            console.log(fieldValue," should be an array");
            console.log(queryField.selectionSet.selections," should exist and be an array");
            //returnObject[queryField.name.value]=fieldValue;
            returnObject[queryField.name.value]=await rebuildArrays(fieldValue,queryField.selectionSet.selections);
